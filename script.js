...
function renderOrdersList() {
    ...
    const statusLabel = {
        'pending': 'Menunggu Pembayaran', 'read': 'Dibaca', 'processing': 'Diproses',
    ...
    container.innerHTML = '<div class="orders-list">' + myOrders.map(o => {
        ...
        const isPaymentPending = o.status === 'pending' || o.status === 'unpaid';
        const paymentBadge = isPaymentPending ? `
            <div class="order-msg-badge" onclick="event.stopPropagation(); window.openPaymentModal('${o.id}')">
        ...
        return `
            <div class="admin-order-item">
                ...
                <div class="admin-order-meta">
                    ...
                    ${isPaymentPending ? '<span style="color:var(--gold);margin-left:8px;"><i class="fas fa-clock"></i> Belum Bayar</span>' : '<span style="color:var(--green);margin-left:8px;"><i class="fas fa-check-circle"></i> Lunas</span>'}
                </div>
                ...
                <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;">
                    ...
                    <div style="display:flex;gap:8px;">
                        ${isPaymentPending ? `<button class="admin-chat-btn" style="background:var(--gold);color:#000;" onclick="window.markOrderPaid('${o.id}')"><i class="fas fa-check"></i> Tandai Lunas</button>` : ''}
...
window.markOrderPaid = function(orderId) {
    ...
    if (order.status === 'pending' || order.status === 'unpaid') {
        ...
        if (order.timeline) {
            order.timeline[0] = { step: 'Pembayaran Diverifikasi', desc: 'Pembayaran telah dikonfirmasi admin', time: new Date().toLocaleString('id-ID'), completed: true };
        }
        ...
        renderAdminOrders();
        updateAdminStats();
        showToast('Berhasil', 'Pesanan ditandai sebagai lunas', 'success');
    }
};

function updateAdminStats() {
    document.getElementById('adminStatTotal').textContent = orders.length;
    document.getElementById('adminStatPending').textContent = orders.filter(o => o.status === 'pending' || o.status === 'unpaid').length;
    document.getElementById('adminStatProcessing').textContent = orders.filter(o => o.status === 'processing' || o.status === 'read').length;
...
function updateOrderStatus(orderId, newStatus) {
    ...
    order.statusLabel = statusLabelMap[newStatus] || newStatus;
    localStorage.setItem('joellOrders', JSON.stringify(orders));
    syncOrdersToCloud();
    updateAdminStats();
    renderAdminOrders();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (checkoutForm) {
        checkoutForm.addEventListener('submit', async function(e) {
            ...
            showToast('✅ Pesanan Dibuat', 'Silakan selesaikan pembayaran', 'success', 3000);
    ...
    if (trackBtn) {
        trackBtn.addEventListener('click', function() {
            ...
            const statusConfig = {
                'pending': { label: 'Menunggu Pembayaran', color: 'var(--gold)', bg: 'rgba(251,191,36,0.15)' },
